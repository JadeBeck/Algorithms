...
/*function countingSort(arr : any) {
    ...
    for (let i = 0 ; i < arr.length ; i++) {
        ...
        if (arr.indexOf(i) !== -1 && arr.lastIndexOf(i) === arr.indexOf(i)) {
            let num = 1;
            array.push(num);
        }
        if (arr.lastIndexOf(i) !== arr.indexOf(i)) {
            let num = arr.lastIndexOf(i) - arr.indexOf(i) + 1;
            array.push(num);
        }
    }
    return array;
...
function countingSort(arr: any) {
    // @ts-ignore
    let counterArray: any = new Array<any>(100).fill(0);
